...
if (-not $SkipK6) {
  Invoke-Step -Name "k6 main load test" -Command "k6 run --summary-export `"$runDir\main-summary.json`" load-tests/main-load-test.js" -LogFile (Join-Path $runDir "04-k6-main.log")
  Invoke-Step -Name "k6 feature matrix load test" -Command "k6 run --summary-export `"$runDir\matrix-summary.json`" load-tests/feature-matrix-load-test.js" -LogFile (Join-Path $runDir "05-k6-matrix.log")
}
